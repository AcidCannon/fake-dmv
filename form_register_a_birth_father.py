...
import form_register_a_birth_father_support

def vp_start_gui():
    '''Starting point when module is the main routine.'''
    global val, w, root
    root = tk.Tk()
    top = form_register_a_birth_father (root)
    form_register_a_birth_father_support.init(root, top)
    msgBox.showinfo('Info', 'Please input additional information (not required) about father, and click \'Apply\'!')
    root.mainloop()

w = None
def create_form_register_a_birth_father(root, *args, **kwargs):
    '''Starting point when module is imported by another program.'''
    global w, w_win, rt
    rt = root
    w = tk.Toplevel (root)
    top = form_register_a_birth_father (w)
    form_register_a_birth_father_support.init(w, top, *args, **kwargs)
    return (w, top)

def destroy_form_register_a_birth_father():
    global w
    w.destroy()
    w = None

class form_register_a_birth_father:
    def __init__(self, top=None):
        ...
        top.title("Agent - Register a birth - Father")
        ...
        self.button_apply.configure(command=form_register_a_birth_father_support.button_apply_click)
